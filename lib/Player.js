...
export class Player {
    constructor(scene) {
        ...
        this.burstTimer = 0;
        this.burstDelay = 0.1; // 100ms between burst bullets
    ...
    update(deltaTime) {
        ...
        this.updateBurstShooting(deltaTime); // Handle burst shooting timing
        this.handleShooting();
    }
    ...
    handleShooting() {
        ...
        if (this.pendingShot && this.shootCooldown <= 0) {
            this.shootSingleBullet(); // Single bullet
            this.shootCooldown = this.shootCooldownTime; // Apply cooldown
            this.pendingShot = false;
        }
    }
    
    updateBurstShooting(deltaTime) {
        if (!this.isBurstShooting) return;
        
        this.burstTimer += deltaTime;
        
        if (this.burstTimer >= this.burstDelay && this.burstShotsFired < this.burstTotalShots) {
            this.shootSingleBullet(); // Fire one bullet
            this.burstShotsFired++;
            this.burstTimer = 0; // Reset timer for next bullet
            
            console.log(`Burst bullet ${this.burstShotsFired}/${this.burstTotalShots}`);
            
            // If this was the last bullet, apply cooldown and end burst
            if (this.burstShotsFired >= this.burstTotalShots) {
                this.isBurstShooting = false;
                this.shootCooldown = this.shootCooldownTime; // Apply full cooldown
                console.log('Burst complete, cooldown applied');
            }
        }
    }
    ...
    handleSpacePress() {
        ...
        if (timeSinceLastPress < this.doubleClickTimeout) {
            ...
            if (this.spaceClickCount >= 2) {
                // Start burst shooting sequence
                this.pendingShot = false; // Cancel any pending single shot
                this.startBurstShooting(2); // Two bullets with delay
                this.spaceClickCount = 0;
                console.log('Double-click burst started!');
                return;
            }
        } else {
    ...
    startBurstShooting(bulletCount) {
        if (this.shootCooldown > 0) return; // Still in cooldown
        
        this.isBurstShooting = true;
        this.burstTimer = 0;
        this.burstShotsFired = 0;
        this.burstTotalShots = bulletCount;
        
        // Fire the first bullet immediately
        this.shootSingleBullet();
        this.burstShotsFired = 1;
        
        console.log(`Burst started: ${this.burstShotsFired}/${this.burstTotalShots}`);
    }
