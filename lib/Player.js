...
export class Player {
    constructor(scene) {
        ...
        // Burst shooting system - always fires 2 bullets in succession
        this.burstCount = 2; // Number of bullets per burst
        this.burstDelay = 0.1; // 100ms between bullets in burst
    ...
    handleShooting(deltaTime) {
        ...
        if (this.burstShotsRemaining > 0) {
            ...
            if (this.burstTimer <= 0) {
                ...
                if (this.burstShotsRemaining > 0) {
                    this.burstTimer = this.burstDelay; // Set timer for next bullet
                }
            }
        }
        
        // Check for new spacebar press
        if (this.keys.space && this.shootCooldown <= 0 && this.burstShotsRemaining === 0) {
            this.startBurstShot();
        }
    }
    ...
    startBurstShot() {
        ...
        this.burstTimer = 0; // Fire first bullet immediately
        this.shootCooldown = this.shootCooldownTime; // Start main cooldown
        console.log('Starting burst shot!');
    }
